...
def bump_project_version(part: str):
    ...
    try:
        ...
        console.print(f"[bold red][ERROR][/bold red] Invalid or missing version string in '{pyproject_path.name}'. Expected format: 'X.Y.Z'")
        sys.exit(1)
    ...
    try:
        ...
        console.print(f"[bold red][ERROR][/bold red] Failed to write updated version to '{pyproject_path.name}': {e}")
        sys.exit(1)
        
    console.print(f"[bold green]     Updating[/bold green] version from [yellow]{old_version}[/yellow] to [cyan]{new_version}[/cyan]")
